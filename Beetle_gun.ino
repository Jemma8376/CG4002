...
unsigned long bounce_data = 0;
const unsigned long bounce_timer = 600;
bool isPrevButtonZero = false; 
...
void shootAmmo(unsigned long currentMillis)
{
  if (!buttonPressed && digitalRead(BUTTON_PIN) == HIGH)
  {
    if (gameState.ammo > 0)
    {
      ...
      sendGun();
      // Serial.println("Shoot button pressed! Sending gun packet...");
      hasSentGun = true;
    ...
    if (currentUpdateState == READING_UPDATE) {
      ...
      switch (incoming) {
        case 'r':  // Reset state
          ...
          break;
        case 'g':
          //Serial.println("Received Gun ACK!");
          // Gun ACK from Python.
...
void loop(){
  ...
  // if (digitalRead(BUTTON_PIN) == LOW)
  // {
  //   if (isPrevButtonZero == false){
  //     bounce_data = currentMillis;
  //   }
  //   if (currentMillis - bounce_data >=  bounce_timer){
  //      buttonPressed = false;
  //   }
  //   isPrevButtonZero = true;
  // }
  // else{
  //   isPrevButtonZero = false;
  // }
  ...
  if (hasSentGun && !hasAcknowledgedGun && (currentMillis - timeoutStart >= TIMEOUT_VAL)) {
      //Serial.println("Resending Gun packet...");    
      resendGunpacket();
      timeoutStart = currentMillis;
  }
...
void sendGun() {
  //Serial.println("Sending Gun packet...");  
